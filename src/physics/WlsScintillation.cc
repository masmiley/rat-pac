...
//
//
// $Id: WlsScintillation.cc,v 1.38 2010-12-15 07:39:26 gunter Exp $
...
// WlsScintillation::operator=(const WlsScintillation &right)
// {
// }
...
WlsScintillation::WlsScintillation(const G4String& processName,
                                       G4ProcessType type)
...
WlsScintillation::~WlsScintillation()
{
...
//
G4VParticleChange*
WlsScintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
        return WlsScintillation::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
WlsScintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
        ...
        G4MaterialPropertyVector* Fast_Intensity = 
                aMaterialPropertiesTable->GetProperty("WLSFASTCOMPONENT"); 
        G4MaterialPropertyVector* Slow_Intensity =
                aMaterialPropertiesTable->GetProperty("WLSSLOWCOMPONENT");
        ...
        if (scintillationByParticleType) {
           ...
           if (!Scint_Yield_Vector) {
             ...
             G4Exception("WlsScintillation::PostStepDoIt","Scint01",
           ...
           ScintillationYield = aMaterialPropertiesTable->
                                      GetConstProperty("WLSSCINTILLATIONYIELD");
        ...
        G4double ResolutionScale    = aMaterialPropertiesTable->
                                      GetConstProperty("WLSRESOLUTIONSCALE");
       ...
       // G4double constBirks = 0.0;
               ...
               if (nscnt == 1) {
                 if(Fast_Intensity){
                   ScintillationTime   = aMaterialPropertiesTable->
                                           GetConstProperty("WLSFASTTIMECONSTANT");
                   if (fFiniteRiseTime) {
                 ...
                 if(Slow_Intensity){
                   ScintillationTime   = aMaterialPropertiesTable->
                                           GetConstProperty("WLSSLOWTIMECONSTANT");
                   if (fFiniteRiseTime) {
                      ScintillationRiseTime = aMaterialPropertiesTable->
                 ...
                 G4double YieldRatio = aMaterialPropertiesTable->
                                          GetConstProperty("WLSYIELDRATIO");
        ...
        if (verboseLevel>0) {
        G4cout << "\n Exiting from WlsScintillation::DoIt -- NumberOfSecondaries = " 
...
void WlsScintillation::BuildThePhysicsTable()
{
...
void WlsScintillation::SetScintillationByParticleType(const G4bool scintType)
{
        if (emSaturation) {
           G4Exception("WlsScintillation::SetScintillationByParticleType", "Scint02",
...
G4double WlsScintillation::GetMeanFreePath(const G4Track&,
                                          G4double ,
...
G4double WlsScintillation::GetMeanLifeTime(const G4Track&,
                                          G4ForceCondition* condition)
{
        *condition = Forced;
...
G4double WlsScintillation::sample_time(G4double tau1, G4double tau2)
{
